...
extern int singTunStart(int tunFd, const char* socksHost, int socksPort, int mtu, const char* username, const char* password);
extern int singTunStop(void);
extern int singTunIsRunning(void);
...
Java_com_simplexray_re_service_TProxyService_SingTunIsRunning(
    ...
    return singTunIsRunning() ? JNI_TRUE : JNI_FALSE;
}
...
Java_com_simplexray_re_service_TProxyService_SingTunGetStats(
    ...
    singTunGetStats(&tx_pkts, &tx_bytes, &rx_pkts, &rx_bytes);
