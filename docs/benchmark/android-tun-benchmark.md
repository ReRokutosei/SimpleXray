...
在 MTU 9000 下，5GHz Wi-Fi 多流下行达到 377.12 Mbps，USB 有线单流上行达 638.93 Mbps，MTU 9000 多流上行 706.84 Mbps；标准 MTU 1500 单流下行维持在 260.96 ~ 303.01 Mbps。在 60 秒持续高吞吐测试中，8 流平均吞吐为 725.95 Mbps，衰减率为 +2.72%，未出现吞吐断崖式下跌。

UDP 传输抖动较低，USB 单流抖动为 0.053 ms，Wi-Fi 8 流抖动为 0.627 ms。USB 有线 8 流 UDP 可达 200 Mbps 目标限速（接近 0% 丢包）；Wi-Fi 环境下单流 UDP 下行在 109.27 ~ 128.39 Mbps。
...
lwIP 单线程事件循环在多流并发打满时存在排队瓶颈。在载荷时延膨胀测试中，8 流并发下行由于单线程 CPU 0 满载，虚拟网卡队列出现数据包积压，时延膨胀达 **+122.70 ms**（对应吞吐 552.3 Mbps）；在 60 秒长跑中段亦出现局部吞吐收窄（变异系数为 19.67%）。启动时需向文件系统写入临时配置文件 `tproxy.conf`，增加一次文件 I/O。
...
多流并发下行性能较优，MTU 1500 下 8 并发 TCP 下行达 402.53 Mbps（Wi-Fi）与 384.82 Mbps（USB），MTU 9000 多流下行在 Wi-Fi 达 409.40 Mbps。在 60 秒持续高吞吐测试中，8 流平均吞吐为 814.26 Mbps，变异系数为 10.24%，未观察到因 Go GC 周期停顿引起的吞吐骤降。

USB 单流 UDP 传输抖动为 0.067 ms。8 流 UDP 在 USB 接近 200 Mbps 目标限速（198.89 ~ 199.67 Mbps）；单流 UDP 上行在 USB 测得 178.89 Mbps。

**资源特性**

通过 JNI 直接接管 `VpnService` fd，无临时配置文件。Scheme 2 独立微基准测得纯栈 TCP 斜率 52.14 KiB/conn，UDP 45.84 KiB/conn。在载荷时延膨胀测试中，8 流并发下行时延膨胀为 **+35.50 ms**（吞吐 866.23 Mbps），优于物理直连基线的 +78.55 ms，反映出用户态多协程平滑缓冲调度的特性。

**局限性**

Go 运行时调度导致高并发时 CPU 峰值达 139 ~ 215%（TCP），UDP 单流峰值达 241%，能效比低于纯 C 实现。纯用户态 TCP 引擎在单流下行场景（~175 Mbps）弱于 Hev（~261 Mbps），受协程调度延迟影响。
...
在 60 秒持续高吞吐测试中，8 流平均吞吐为 855.09 Mbps，稳定性变异系数为 **6.25%**（在四种协议栈中最低），时序曲线波动幅度最小，体现了内置 BBRv3 在高带宽稳态传输下的速率控制特性。

USB 有线环境下 MTU 1500 单流上行达 520.22 Mbps，MTU 9000 多流上行达 673.71 Mbps，多流下行 387.04 Mbps。8 流 UDP 下行测得 182.22 ~ 198.50 Mbps，上行达到 199.72 Mbps。

**资源特性**

完全基于非阻塞 epoll/netpoller 实现，无外部 C 依赖。单流载荷时延膨胀增量接近 0，8 流并发膨胀为 +52.50 ms。Scheme 2 测量 TCP 纯栈斜率 52.66 KiB/conn（与端到端 R2+R3 均值 52.33 KiB/conn 吻合）。

**局限性**

MTU 1500 单流场景平均 CPU 开销为 39.2% ~ 84.7%，峰值达 200%；建议在支持大帧的环境下启用大 MTU。UDP 纯底层斜率在四种后端中最高（113.88 KiB/conn，受 BBRv3 环形缓冲区分配影响），Android 端在 GC 稳态下有所回落。
...
在 60 秒持续测试中平均吞吐为 856.14 Mbps，变异系数为 8.80%，衰减率为 +1.14%。标准单流下行在 Wi-Fi 与 USB 环境下均处于 58.13 ~ 63.33 Mbps，下行受限主要受 gVisor 单核事件循环与内部 buffer 拷贝调度制约。8 流 UDP 上下行均达到 197.41 ~ 200.14 Mbps。

**资源特性**

在载荷时延膨胀测试中，8 流下行并发时延膨胀为 **+34.40 ms**（吞吐 858.98 Mbps），与 SingTUN 处于相近水平，表明 gVisor 内部虚拟流控队列对突发交互数据包的排队挤压较小。主进程连接增长斜率仅为 2.15 KiB/conn（TCP）与 3.24 KiB/conn（UDP），Scheme 2 测量子进程内部 gVisor 真实开销为 63.54 KiB/conn（TCP）与 57.55 KiB/conn（UDP）。

**局限性**

单流下行约 60 Mbps 时平均 CPU 占用仍达 65.4% ~ 96.1%，峰值约 172% ~ 199%，单位吞吐算力消耗偏高。
...
| 协议栈后端 | 适用场景 | 优势 | 代价 |
| :--- | :--- | :--- | :--- |
| **`hev-socks5-tunnel`** | 日常轻量续航、低功耗保活、发热敏感环境 | CPU 占用低、能效比高、纯底层开销仅 12.59 KiB/conn、UDP 抖动较低、长跑衰减率低 (+2.7%) | MTU 1500 多流下行受限（~290 Mbps）、8 流高并发缓冲膨胀较大 (+122.7 ms)、启动需写临时文件 |
| **`SingTUN`** | 高速无线 Wi-Fi、大带宽流媒体、多流并发下载 | 多流下行并发性能好、高并发下载时延膨胀较小 (+35.5 ms)、60s 持续吞吐稳定 (814 Mbps)、无临时文件 I/O | 高并发 CPU 占用偏高、单流下行弱于 Hev、基础内存占用偏高 |
| **`MipsTUN`** | 有线网络、局域网大吞吐、巨型帧环境 | 有线上行吞吐高、内置 BBRv3、60s 持续吞吐波动最小 (CV 6.25%)、纯 Go 架构无 C 依赖 | 标准 MTU 下 CPU 负载较重、UDP 纯栈内存斜率偏高 (113.88 KiB/conn) |
| **`Xray Native TUN`** | 主进程内存隔离、进程级防护、原生核心直驱 | 主进程内存增长极低（2.15 KiB/conn）、高并发下载时延膨胀较小 (+34.4 ms)、进程级隔离 | 标准单流下行吞吐受限、单位吞吐能效比偏低 |
