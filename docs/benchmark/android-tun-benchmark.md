...
综合三轮测试数据，各协议栈的性能与资源特征如下：

### 5.1 `hev-socks5-tunnel` (C / lwIP 协议栈)

* **吞吐表现**：
  - 在 **MTU 8500** 巨型帧模式下，5GHz Wi-Fi 下行达到 **382 \~ 473 Mbps**，USB 3.2 有线下行达到 **362 \~ 404 Mbps**，接近当前物理链路的实测带宽。
  - 在 **MTU 1500** 单流模式下，下行吞吐维持在 **250 \~ 345 Mbps**，表现较为稳定。
* **资源开销与优势**：
  - **CPU 占用与发热控制良好**：得益于纯 C 语言编写与裁剪版 lwIP，常规单流及多流传输下的平均 CPU 占用率为 **14% \~ 38%**，峰值处于 **55% \~ 97%**，处理器负载与功耗表现相对较低。
  - **内存占用平稳**：PSS 稳定在 **94 \~ 124 MB** 区间，无垃圾回收机制带来的内存抖动。
* **主要局限**：
  - **多流下行吞吐受限**：受限于 lwIP 单线程事件循环架构，在 MTU 1500 多流（P=8）并发下行时出现吞吐瓶颈，Wi-Fi 下行收窄至 **\~250 Mbps**，未充分利用多核 CPU 算力进一步提升吞吐。
  - **工程维护成本较高**：依赖大量 C 语言子模块源码与 CMake 交叉编译工具链；启动时需向磁盘写入临时配置文件 `tproxy.conf`，增加了一次文件 I/O 开销。

### 5.2 `SingTUN` (Go / sing-box 专有自研栈)

* **吞吐表现**：
  - **多流高并发吞吐表现良好**：在 **MTU 1500** 默认帧下，8 并发下行吞吐达到 **405 \~ 469 Mbps**（Wi-Fi）与 **372 \~ 399 Mbps**（USB），较 Hev 同配置高出约 **60% \~ 80%**，多核并发优势明显。
  - 在 **MTU 8500** 模式下，单流与多流下行均可达到 **450 \~ 470 Mbps**，接近无线测试链路带宽上限。
* **资源开销与优势**：
  - 核心转发路径基于 Slab 池化与无锁环形缓冲区管理内存，空闲及常规单流传输下常驻内存处于 **106 \~ 128 MB** 之间，相比 Hev 增量不足 10 MB。
  - 架构内聚度高，通过 JNI 直接接管 `VpnService` 的文件描述符（`vpnFd`），消除了临时配置文件与外部进程启动开销。
* **主要局限**：
  - **高负载 CPU 占用较高**：在高并发吞吐下，多核协程调度与 Go 运行时带来了一定的算力开销。单流平均 CPU 为 **21% \~ 42%**；在 8 流并发冲刺 470 Mbps 时，多核峰值占用达到 **120% \~ 250%**，设备持续发热与电量消耗高于纯 C 实现。
  - **峰值内存动态上升**：在高并发冲击下，Go 运行时堆内存存在短时扩容现象，PSS 会上升至 **142 \~ 155 MB**，虽能被 GC 及时回收，但峰值占用仍高于纯 C 实现。
  - **新栈成熟度需持续观察**：作为较新引入的协议栈，在不同网络环境下的长期稳定性仍需持续验证。

### 5.3 `Xray Native TUN` (Go / gVisor netstack)

* **吞吐表现**：
  - **下行性能存在明显瓶颈**：单流下行在 Wi-Fi 与 USB 有线环境下均处于 **54 \~ 95 Mbps** 水平；8 并发下行为 **136 \~ 181 Mbps**，与其他协议栈存在明显差距。
* **资源开销与技术分析**：
  - **CPU 占用与吞吐产出不匹配**：在下行吞吐仅 50 \~ 60 Mbps 的情况下，平均 CPU 占用率仍处于 **50% \~ 100%**，多流并发峰值达 **170% \~ 374%**。
  - **猜测原因**：Xray 原生 TUN 所集成的 Google gVisor 网络栈设计初衷为云原生容器的安全隔离，并非专为移动端低功耗网络转发优化。在 Android 上处理密集网络小包时，高频的用户态上下文切换、切片内存分配与垃圾回收带来了较大的计算开销。

### 5.4 `MipsTUN` (Go / Mihomo mipstack 纯 Go 协议栈)

* **吞吐表现**：
  - **USB 有线上行吞吐突出**：在 USB 3.2 Gen1 有线以太网环境下，MipsTUN 上行表现最为突出。单流在 MTU 1500 下达到 **582.56 Mbps**，在 MTU 8500 下达到 **655.88 Mbps**；8 流并发稳定在 **657 \~ 668 Mbps**，达到物理基准带宽的 90% \~ 93%。
  - **下行性能均衡**：在 MTU 8500 巨型帧下，单流下行达到 **326.94 Mbps**，8 流达到 **372.83 Mbps**，与 Hev 处于同等水平，显著高于 Xray 原生 TUN。
  - **无线网络传输平稳**：在 5GHz Wi-Fi 下，MTU 8500 单流上行达到 **302.91 Mbps**（多流 **327.67 Mbps**），下行达到 **258.69 Mbps**（多流 **282.51 Mbps**）。mipstack 内置的 BBRv3 算法在无线信道下表现出相对平稳的传输能力。
  - **设备内部回环达 20.78 Gbps**：在本地回环压力测试中，MipsTUN 测得 **20.78 Gbps (20,775 Mbps)**，接近系统套接字本地回环水平，表明协议栈自身在纯数据流转路径上未表现出架构级瓶颈。
* **资源开销与优势**：
  - **集成架构简洁**：直接利用 Linux netpoller (epoll) 接管 Android `VpnService` 的非阻塞文件描述符，无临时配置文件与外部进程 IPC 开销。
  - **内存占用平稳**：测试各轮次常驻内存保持在 **115 \~ 156 MB** 区间，压测结束后内存回收及时，未见明显的协程堆积或内存泄漏。
* **主要局限**：
  - **小包 CPU 开销较高**：在 MTU 1500 默认帧下，单流平均 CPU 为 **40% \~ 110%**，峰值达 **190% \~ 240%**；开启 MTU 8500 后，平均 CPU 降至 **31% \~ 64%**。这表明协议栈在应对密集小数据包时，单包处理的 CPU 消耗相对明显。
  - **MTU 1500 多流下行收窄**：在 MTU 1500 且 8 并发下行时，USB 与 Wi-Fi 吞吐收窄至 **145 \~ 166 Mbps**，在支持巨型帧的网络环境下建议适当调大 MTU。
