...
| **Xray TUN (MTU 1500) [Single Stream]** | `xray` | 1500 | TCP | 单流 | 602.33 Mbps | 185.41 Mbps | 157.3% / 170.2% | 165.3 MB |
...
| **Xray TUN (MTU 9000) [Single Stream]** | `xray` | 9000 | TCP | 单流 | 538.48 Mbps | 203.93 Mbps | 146.3% / 177.7% | 166.4 MB |
...
| **Xray TUN (MTU 1500) [P=8]** | `xray` | 1500 | TCP | P=8 | 657.11 Mbps | 513.91 Mbps | 174.3% / 415.7% | 165.9 MB |
...
| **Xray TUN (MTU 9000) [P=8]** | `xray` | 9000 | TCP | P=8 | 588.48 Mbps | 486.38 Mbps | 158.0% / 393.0% | 167.8 MB |
...
> [!NOTE]
> **测试方法说明**：载荷时延膨胀衡量的是链路满载时交互式通信的延迟恶化（$\Delta RTT = L_{loaded} - L_{idle}$）。在 5GHz Wi-Fi、标准 MTU 1500 环境下持续 TCP 下行打流 10 秒，并发每 200ms 发起 TCP echo RTT 探针；探针与被测流量经过同一 Android/TUN 数据路径，直接取空载与受载 RTT 的中位数。TCP 探针替代 ICMP，避免 ICMP 未被某些用户态协议栈接管而产生伪零值。图表数据为 3 轮实测的算术平均值。

#### (9) 60秒持续高吞吐长跑稳定性曲线
![Long-Run Stability Dashboard](../charts/long_run_stability_dashboard.webp)

> [!NOTE]
> **测试方法说明**：本测试同样未采用多轮平均平滑处理，改为单次 60 秒连续长程高并发打流（Wi-Fi 5GHz, TCP MTU 1500, P=8），每 1 秒输出 1 次瞬时吞吐切片。专门用于捕获短跑测试无法暴露的动态特征，包括 Go 运行时周期性 GC 顿挫、TCP 拥塞控制稳态收敛过程、单线程处理器的算力疲劳，以及持续满载下的末段吞吐变化率与变异系数。
...
在 Wi-Fi 6 路由器网关测试中，MTU 9000 下 8 流并发下行达到 852.84 Mbps，上行为 780.74 Mbps；标准 MTU 1500 单流上行达到 698.33 Mbps，下行为 504.30 Mbps。在 USB 3.2 有线环境下，单流上行为 638.93 Mbps，MTU 9000 多流上行为 706.84 Mbps。在 60 秒持续高吞吐测试中，8 流平均吞吐为 725.95 Mbps，末段较首段上升 6.46%，未出现吞吐断崖式下跌。
...
lwIP 采用单线程事件循环，在多流并发打满时存在排队瓶颈。Wi-Fi MTU 1500 下 8 流下行受单线程调度影响收敛于 281.62 Mbps，而上行达 734.86 Mbps。在载荷时延膨胀测试中，8 流并发下行时延膨胀为 +29.70 ms（对应吞吐 254.9 Mbps），长跑中段出现局部吞吐收窄（变异系数 19.67%）。启动时需要向文件系统写入临时配置文件 `tproxy.conf`。
...
通过 JNI 直接接管 `VpnService` 文件描述符，无需临时配置文件。独立微基准测得 TCP 纯栈斜率为 52.14 KiB/conn，UDP 为 45.84 KiB/conn。在载荷时延膨胀测试中，8 流并发下行时延膨胀为 +59.66 ms（吞吐 856.8 Mbps），低于物理直连基线的 +74.27 ms。
...
基于非阻塞 epoll 与 netpoller 实现，无外部 C 依赖。单流和 8 流载荷时延膨胀均未观测到额外增量，受载 RTT 中位数不高于空载。独立微基准测得 TCP 纯栈斜率为 52.66 KiB/conn。
...
在 60 秒持续测试中平均吞吐为 856.14 Mbps，变异系数为 8.80%，末段较首段上升 1.27%。Wi-Fi MTU 1500/9000 单流下行达到 185.41/203.93 Mbps，八流下行达到 513.91/486.38 Mbps；八流下行平均 CPU 约 368%，峰值达到 393.0 ~ 415.7%。8 流 UDP 上下行均达到 197.41 ~ 200.25 Mbps。

**资源特性**

在载荷时延膨胀测试中，8 流下行并发时延膨胀为 +34.78 ms（吞吐 546.6 Mbps）。主进程连接增长斜率仅为 2.15 KiB/conn（TCP）与 3.24 KiB/conn（UDP），独立微基准测得子进程内部 gVisor 真实开销为 63.54 KiB/conn（TCP）与 57.55 KiB/conn（UDP）。

**局限性**

单流下行仍只有 185 ~ 204 Mbps，八流下行平均 CPU 约 368%，单位吞吐算力消耗高于其他方案。
...
| 协议栈后端 | 适用场景 | 优势 | 代价 |
| :--- | :--- | :--- | :--- |
| **`hev-socks5-tunnel`** | 日常轻量续航、低功耗保活、发热敏感环境 | CPU 占用低、能效比高、纯底层开销仅 12.59 KiB/conn、UDP 抖动较低、长跑末段上升 6.5% | MTU 1500 多流下行受限（~282 Mbps）、8 流时延膨胀 +29.7 ms、启动需写临时文件 |
| **`SingTUN`** | 高速无线 Wi-Fi、大带宽下载、多流并发 | 多流下行并发性能强（810 ~ 862 Mbps）、高并发下载时延膨胀 +59.7 ms、60s 持续吞吐稳定 (814 Mbps)、无临时文件 I/O | 高并发与 UDP 场景 CPU 占用较高 (380%~430%)、MTU 1500 单流下行弱于 Hev、基础内存占用偏高 |
...
| **`Xray Native TUN`** | 主进程内存隔离、进程级防护、原生核心直驱 | 主进程内存增长极低（2.15 KiB/conn）、Wi-Fi 八流下行 486 ~ 514 Mbps、高并发下载时延膨胀 +34.8 ms、进程级隔离 | 单流下行 185 ~ 204 Mbps、八流下行平均 CPU 约 368%、单位吞吐能效比偏低 |
