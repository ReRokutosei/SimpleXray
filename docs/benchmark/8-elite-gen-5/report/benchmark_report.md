...
- **UDP Saturation Ladder**: 600 Mbps multi-stream UDP ($P=8$, 75 Mbps × 8 streams) achieved full line rate with 0.0% packet loss across all four backends. Single-stream UDP ($P=1$) at 600 Mbps reached line-rate download with modest loss (9.2%–14.7% on Hev, Zeptun, and SimpleTUN), while upload hit expected single-socket queue saturation limits (~34%–52% loss).
