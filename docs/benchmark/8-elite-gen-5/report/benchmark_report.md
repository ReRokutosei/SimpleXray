...
| :--- | :--- | :--- | :--- | :--- |
| **TCP $P=1$ (Single Stream)** | Download | 413.3 Mbps | 385.7 Mbps | 442.5 Mbps |
| | Upload | 453.8 Mbps | 446.4 Mbps | 510.9 Mbps |
| **TCP $P=8$ (Multi-Stream)** | Download | 450.3 Mbps | 368.5 Mbps | 444.2 Mbps |
| | Upload | 475.5 Mbps | 469.4 Mbps | 479.7 Mbps |
| **UDP $P=1$ (Single Stream, 600M Target)** | Download | 599.4 Mbps (9.2% loss) | 600.0 Mbps (22.2% loss) | 600.0 Mbps (12.3% loss) |
| | Upload | 600.0 Mbps (37.9% loss) | 600.0 Mbps (34.3% loss) | 600.0 Mbps (38.2% loss) |
| **UDP $P=8$ (Multi-Stream, 600M Aggregate)** | Download | 75.0 Mbps × 8 (0.0% loss) | 75.0 Mbps × 8 (0.0% loss) | 75.0 Mbps × 8 (0.3% loss) |
| | Upload | 75.0 Mbps × 8 (0.0% loss) | 75.0 Mbps × 8 (0.0% loss) | 75.0 Mbps × 8 (0.0% loss) |

- In TCP workloads, Zeptun recorded the highest throughput across both single-stream ($P=1$) download (442.5 Mbps) and upload (510.9 Mbps). Hev achieved 413.3 Mbps download and 453.8 Mbps upload, while SingTUN recorded 385.7 Mbps download and 446.4 Mbps upload.
- In multi-stream TCP ($P=8$), Hev maintained 450.3 Mbps download and 475.5 Mbps upload without suffering from the single-threaded degradation observed on lower-power silicon, driven by the higher single-core frequency of the SM8850 Prime cores. SingTUN recorded 368.5 Mbps download, and Zeptun recorded 444.2 Mbps download.
- In single-stream UDP ($P=1$, 600 Mbps target rate), download line rate reached 599.4–600.0 Mbps across all backends. Download loss was lowest on Hev (9.2%) and Zeptun (12.3%), while SingTUN recorded 22.2% loss. For upload, all three backends saturated at 600.0 Mbps target rate with 34%–38% packet loss due to single-socket transmit queue bottlenecks without flow control.
- In multi-stream parallel UDP ($P=8$, 75 Mbps × 8 streams = 600 Mbps aggregate), all backends achieved full line rate with 0.0% packet loss (Zeptun download loss was 0.3%), proving that multi-core distribution resolves the single-socket TX queue bottleneck.
...
| Scenario | Direction | Hev (C / lwIP) | SingTUN (Go) | Zeptun (Zig) |
| :--- | :--- | :--- | :--- | :--- |
| **$P=1$ (Single Stream)** | Upload | 5.3% | 8.0% | 4.7% |
| | Download | 11.0% | 17.0% | 9.7% |
| **$P=8$ (Multi-Stream)** | Upload | 6.5% | 9.3% | 5.9% |
| | Download | 15.2% | 32.5% | 16.9% |

- On the Snapdragon 8 Elite Gen 5, Zeptun demonstrated the lowest CPU compute cost in single-stream transport, requiring 4.7% CPU per 100 Mbps in upload and 9.7% CPU in download. Hev followed closely with 5.3% CPU (upload) and 11.0% CPU (download).
- In multi-stream ($P=8$) download, Hev recorded 15.2% CPU per 100 Mbps, Zeptun recorded 16.9% CPU per 100 Mbps, and SingTUN recorded 32.5% CPU per 100 Mbps.
...
- Under heavy background 8-stream TCP download (431–490 Mbps), loaded latency increased by +124.2 ms on SingTUN, +125.0 ms on Zeptun, and +131.4 ms on Hev.
- Latency inflation remained closely matched across all three implementations under equivalent wireless channel saturation.
...
| :--- | :--- | :--- | :--- | :--- |
| **3% Loss** | Upload | 447.9 Mbps | 455.2 Mbps | 483.1 Mbps |
| | Download | 17.0 Mbps | 15.9 Mbps | 13.9 Mbps |
| **5% Loss** | Upload | 541.0 Mbps | 479.9 Mbps | 401.7 Mbps |
| | Download | 11.1 Mbps | 8.7 Mbps | 9.2 Mbps |
| **8% Loss** | Upload | 454.3 Mbps | 382.1 Mbps | 454.6 Mbps |
| | Download | 6.4 Mbps | 6.7 Mbps | 5.9 Mbps |
...
- Download throughput exhibited expected sensitivity to client-side drops over the 50 ms RTT delay, declining smoothly from ~14–17 Mbps (3% loss) down to ~6 Mbps (8% loss) across all three backends.
...
- **Flagship Single-Core Performance & Event Loop Scaling**: On Snapdragon 8 Elite Gen 5 (SM8850), Hev sustained 450.3 Mbps in multi-stream download and 498.6 Mbps in the 60-second stability test without the severe drop observed on lower-power mid-range cores. The higher IPC and single-core clock of the Prime cores reduce packet processing bottlenecks in single-threaded event loops.
- **Multi-Stream UDP Parallelism**: While single-stream UDP ($P=1$) at 600 Mbps experienced 34%–38% packet loss during upload due to single-socket TX buffer saturation without backpressure, multi-stream parallel UDP ($P=8$, 75 Mbps × 8 streams) completed with 0.0% packet loss across all backends. Parallelism spreads socket buffers across independent worker routines and kernel queues, achieving line rate cleanly.
- **Compute Efficiency on High-Performance Silicon**: Zeptun achieved the lowest CPU cost in single-stream workloads (4.7%–9.7% CPU per 100 Mbps). SingTUN's multi-stream compute cost on SM8850 dropped to 32.5% CPU per 100 Mbps (down from 46.5% on 778G), demonstrating effective scaling with modern multi-core architectures.
- **Short-Lived Connection Scaling (CPS)**: SingTUN scaled from 197.2 conn/s (4W) to 643.5 conn/s (8W), and Zeptun scaled from 190.0 conn/s (4W) to 538.9 conn/s (8W). Both backends benefited from 8-worker concurrency, whereas Hev was limited by fixed lwIP PCB allocations.
...
- **Zeptun (Zig / userspace)** recorded top TCP throughput across single-stream download (442.5 Mbps) and upload (510.9 Mbps), maintaining the lowest compute cost in single-stream workloads (4.7% upload / 9.7% download per 100 Mbps) and 538.9 conn/s at 8 workers.
- **UDP Saturation Ladder**: 600 Mbps multi-stream UDP ($P=8$, 75 Mbps × 8 streams) achieved full line rate with 0.0% packet loss across all three backends. Single-stream UDP ($P=1$) at 600 Mbps reached line-rate download with modest loss (9.2%–12.3% on Hev/Zeptun), while upload hit expected single-socket queue saturation limits (~34%–38% loss).
