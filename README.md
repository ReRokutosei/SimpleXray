...
| :------------------------------ | :---------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Process & Execution**         | `ProcessBuilder` STDIN pipe execution                       | Standardized `ProcessBuilder` STDIN pipe execution with clean multi-ABI CMake builds                                                                                                       |
| **Traffic & IPC Control**       | Loopback SOCKS5 proxying & random `127.X.X.X` gRPC ports    | Optimized Unix Domain Sockets (UDS) / IPC IPC bus & loopback gRPC                                                                                                                          |
| **Config Processing**           | Single-format JSON manual import without conflict checks    | Supports JSON and **YAML** formats via Storage Access Framework (SAF), with auto-sanitization for `tun`/SOCKS port conflicts and in-app searchable config editor                           |
...
| **UI & Experience**             | Standard Material 3 UI layout                               | Modern Xiaomi HyperOS / MIUI design system powered by `compose-miuix-ui` (Squircle corners, fluid spring physics, Miuix TopAppBar, Cards, OverlayDialogs, BottomSheets) |
...
### 1. Xiaomi HyperOS / MIUIX UI Refactoring
- **MIUIX Design Language**: Complete overhaul of the user interface using [`compose-miuix-ui`](https://github.com/compose-miuix-ui/miuix), delivering Xiaomi HyperOS / MIUI aesthetic styling.
- **Squircle & Spring Physics**: Integrated Miuix Squircle corner shapes, dynamic theme controllers (Light/Dark/Auto), and spring-animated UI elements.
- **Interactive Componentry**: Replaced standard controls with Miuix `TopAppBar`, `Card`, `InputField`, `Checkbox`, `OverlayIconDropdownMenu`, `OverlayDialog`, and `OverlayBottomSheet`.

### 2. Multi-Format Configuration & SAF Import
...
### 3. Custom Rule File (`.dat`) Management & Arbitrary Dat Support
...
### 4. Process Management & IPC Modernization
...
### 6. AST Config Sanitization & Inbound Safety
...
### 8. Core Component & Submodule Upgrades
...
### 9. Build System & Dependency Modernization
...
The compiled APK will be generated at `app/build/outputs/apk/release/simplexray-arm64-v8a.apk`.

---

## Acknowledgements & Credits

This project builds upon and integrates the work of several fantastic open-source projects:

- **[compose-miuix-ui](https://github.com/compose-miuix-ui/miuix)** - Beautiful Xiaomi HyperOS / MIUI styled Jetpack Compose UI component library for Kotlin Multiplatform.
- **[Xray-core](https://github.com/XTLS/Xray-core)** - High-performance core network proxy engine.
- **[SimpleXray](https://github.com/lhear/SimpleXray)** - Original lightweight Android client upstream.
- **[hev-socks5-tunnel](https://github.com/heiher/hev-socks5-tunnel)** - High-performance SOCKS5 VPN tunnel implementation for Android.
